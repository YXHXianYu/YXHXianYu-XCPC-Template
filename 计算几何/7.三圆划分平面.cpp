...
void work() {
    ...
    if(n == 1) {
        ...
        int v = circleIntersect(a[1].first, a[1].second, a[2].first, a[2].second);
        if(v == 2) {
            cout << 4 << endl;
        } else {
            cout << 3 << endl;
    ...
    int emptyCnt = 0;
    for(int i = 1; i <= n; i++) {
        auto iis = getCC(a[i].first, a[i].second, a[i%n+1].first, a[i%n+1].second);
        for(auto& x: iis) {
            is.push_back(x);
        }
        if(iis.size() == 0) emptyCnt += 1;
        // cout << i << ", " << i % n + 1 << ": " << endl;
        // for(auto& x: iis)
        //     cout << "(" << x.x << ", " << x.y << ") ";
        // cout << endl;
    ...
    for(int i = 1; i <= n; i++) {
        int cnt = 0;
        for(auto &x: is) {
            if(cmp(a[i].first.distTo(x), a[i].second) == 0)
                cnt++;
        }
        if(cnt == 0) C++, V++;
        cnt = max(1ll, cnt);
