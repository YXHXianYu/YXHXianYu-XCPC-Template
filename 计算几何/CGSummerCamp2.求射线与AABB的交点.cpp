...
void work() {
    ...
    auto preGetIntersectionOfEdgeAndRay = [&](const P& p1, const P& p2, const Ray& ray) -> bool {
        P o = ray[0];
        P x = ray[0] + ray[1];
        ...
        // same line (4)
        if (crossOp(o, x, p1) == 0 && crossOp(o, x, p2) == 0) { return false; }
        return true;
    };

    auto getIntersectionOfEdgeAndRay = [&](const P& p1, const P& p2, const Ray& ray) -> std::pair<bool, P> {
        P o = ray[0];
        P x = ray[0] + ray[1];

        // parellel (1)
        if (!isLL(o, x, p1, p2)) { return std::make_pair(false, P()); }

        // get intersection
        P inter = getLL(o, x, p1, p2);

        // inter isn't in segment (1)
        if (!isMiddle(inter, p1, p2)) { return std::make_pair(false, P()); }
        if (sign(ray[1].x) != cmp(inter.x, ray[0].x)) { return std::make_pair(false, P()); }
        if (sign(ray[1].y) != cmp(inter.y, ray[0].y)) { return std::make_pair(false, P()); }

        // (2)
        return std::make_pair(true, inter);
    };

    auto getIntersectionOfAABBAndRay = [&](const AABB& aabb, const Ray& ray) -> std::vector<P> {
        P p[4] = {aabb[0], P(aabb[0].x, aabb[1].y), aabb[1], P(aabb[1].x, aabb[0].y)};

        std::vector<P> ans;
        for (size_t i = 0; i < 4; i++) {
            auto res = preGetIntersectionOfEdgeAndRay(p[i], p[(i + 1) % 4], ray);
            if (!res) { return ans; } // (4)
        }
        for (size_t i = 0; i < 4; i++) {
        ...
        stable_sort(ans.begin(), ans.end(), [&](const P& a, const P& b) { return cmp(ray[0].distTo(a), ray[0].distTo(b)) < 0; });
    ...
    for (size_t i = 0; i < m; i++) {
        ...
        if (ans.size() > 0) {
            stable_sort(ans.begin(), ans.end(), [&](const P& a, const P& b) { return cmp(ray[i][0].distTo(a), ray[i][0].distTo(b)) < 0; });
            // ans.erase(std::unique(ans.begin(), ans.end()), ans.end());
