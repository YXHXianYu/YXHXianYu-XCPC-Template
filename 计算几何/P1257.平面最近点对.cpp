...
void work() {
    ...
    function<void(int, int)> solve = [&](int L, int R) {
        ...
        if(R - L <= 4) {
            for(int i = L; i <= R; i++) {
                for(int j = i + 1; j <= R; j++) {
                    ans = min(ans, a[i].distTo(a[j]));
                }
            }
            sort(a.begin() + L, a.begin() + R + 1, [](const auto& a, const auto& b) {
                return a.y < b.y;
            });
        }
        ...
        vector<P> t;
        for(int i = L; i <= R; i++) {
            if(cmp(min(abs(a[i].x - midx), abs(a[i].x - midx2)), ans) == -1) {
                for(int j = t.size() - 1; j >= 0 && cmp(a[i].y - t[j].y, ans) == -1; j--) {
                    if(t[j].distTo(a[i]) < ans) ans = t[j].distTo(a[i]);
                }
                t.push_back(a[i]);
            }
        }
