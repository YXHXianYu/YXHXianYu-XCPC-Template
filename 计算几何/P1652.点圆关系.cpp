...
void work() {
    ...
    vector<pair<P, db>> a(n + 1);
    for(int i = 1; i <= n; i++) cin >> a[i].first.x;
    for(int i = 1; i <= n; i++) cin >> a[i].first.y;
    for(int i = 1; i <= n; i++) cin >> a[i].second;

    P b, c;
    b.read(); c.read();

    auto inCircle = [](P o, db r, P p) {
        if(o.distTo(p) <= r) return true;
        else return false;
    };

    int ans = 0;
    for(int i = 1; i <= n; i++) {
        bool bin = inCircle(a[i].first, a[i].second, b);
        bool cin = inCircle(a[i].first, a[i].second, c);
        if(bin ^ cin) ans++;
    }
    cout << ans << endl;
}
