...
void work() {
    ...
    auto solve = [&](int idx) {
        ...
        for(int i = 1; i <= n; i++) {
            if(i != idx) {
                b.push_back(a[i] - a[idx]);
            }
        }
        sort(b.begin(), b.end(), [&](P& p, P& q) {
            int qa = p.quad(), qb = q.quad();
            if(qa != qb) return qa < qb;
            else return sign(p.det(q)) > 0;
        });
        ...
        for(int i = 0; i < n1; i++) {
            ...
            auto check1 = [&]() { // >= pi/2
                if(b[i].dot(b[L]) <= 0) return false;
                int v = b[i].det(b[L]);
                if(v != 0) return v > 0;
                return L < n1;
            };
            while(L - i < n1 && check1()) L++;

            auto check2 = [&]() { // > pi
                int v = b[i].det(b[R]);
                if(v != 0) return v > 0;
                if(b[i].dot(b[R]) > 0)
                    return R < n1; // 正向共线，确保不会跑两遍
                else
                    return R < n1; // 反向共线，只算 i 和 R 均 <n1 的情况
            };
            while(R - i < n1 && check2()) R++; // > pi
