...
void work() {
	...
	for(int i = 1; i <= n; i++) for(int j = i + 1; j <= n; j++) {
		vector<P> q; // intersection
		for(int k = 1; k <= n; k++) {
			P u = p[k];
			P v = p[k % n + 1];
			if(crossOp(p[i], p[j], u) == 0) {
				q.push_back(u);
			}
			if(crossOp(p[i], p[j], u) * crossOp(p[i], p[j], v) < 0) {
				q.push_back(getLL(p[i], p[j], u, v));
			}
		}
		...
		// q index start from 0
		int m = q.size();

		if(q[0].distTo(q[m - 1]) < ans) continue;

		int s = lower_bound(q.begin(), q.end(), p[i]) - q.begin();
		int t = lower_bound(q.begin(), q.end(), p[j]) - q.begin();
		if(s > t) swap(s, t);
